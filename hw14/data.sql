-- 1. Full text search.
UPDATE otus.product SET description = 'MacOS is the most advanced desktop operating system in the world. And with macOS Sonoma, work and play on your Mac are even more powerful — with new ways to elevate your video presentations, boost your gaming performance, and personalize your device.' WHERE name = 'Macbook M2 Pro';
UPDATE otus.product SET description = 'The iPhone 15 and iPhone 15 Plus are smartphones designed, developed, and marketed by Apple Inc. They are the seventeenth and current generation of iPhones, succeeding the iPhone 14 and iPhone 14 Plus. The devices were announced on September 12, 2023, during the Apple Event at Apple Park in Cupertino, California alongside the higher-priced iPhone 15 Pro and 15 Pro Max.' WHERE name = 'iPhone 15';
UPDATE otus.product SET description = 'The IdeaPad 1i raises the bar on every affordable laptop with a super-efficient up-to-12th Generation Intel processor in a thin and compact 17.9 mm chassis that makes multitasking a breeze while boosting energy efficiency with up-to-11 hours of battery life and rapid charge. Get more value with four-sided narrow bezels for maximum screen, Dolby Audio speakers and Flip to Start for instant bootup. Optimize your video call experience with a 1MP camera that comes with a privacy shutter to keep out prying eyes and Smart Noise Cancelling to cut out background noise.' WHERE name = 'Lenovo IdeaPad 1 14IGL7';
UPDATE otus.product SET description = 'The Samsung Galaxy A24 is an Android-based smartphone designed, developed and marketed by Samsung Electronics as a part of its Galaxy A series. This phone was announced on April 19, 2023.' WHERE name = 'Samsung Galaxy A24';
UPDATE otus.product p
    INNER JOIN manufacturer m on p.manufacturer_fk = m.id
    INNER JOIN product_category_ref prc ON prc.product_fk = p.id
    INNER JOIN product_category pc on prc.product_category_fk = pc.id
SET p.full_text_search = CONCAT(p.name, '.', cast(p.description as char(10000)), m.name, '.', pc.name);
--2. History index
